...
<#
.SYNOPSIS
Enables a specified user's account in Active Directory.

.DESCRIPTION
This script enables an Active Directory user account. It requires the user's name and optionally the domain name
...
.PARAMETER UserName
Specifies the username of the Active Directory account to be enabled. This parameter is mandatory.
...
.EXAMPLE
PS> .\Enable-AdUserAccount.ps1 -UserName "jdoe" -DomainName "corp.contoso.com"

This example enables the account of the user 'jdoe' in the 'corp.contoso.com' domain.

.EXAMPLE
PS> .\Enable-AdUserAccount.ps1 -UserName "jdoe" -Credential (Get-Credential)

This example prompts for credentials and enables the account of 'jdoe' in the domain of the executing machine.
...
try {
    ...
    if ($PSBoundParameters.ContainsKey('Credential')) {
        $enableAdUserParams.Credential = $Credential
    }
    Enable-ADAccount @enableAdUserParams
} catch {
    throw "An error occurred while enabling the user account '$UserName': $_"
}